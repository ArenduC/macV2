...
class ExtraExpendView extends StatefulWidget {
  final Function(List<ExtraExpendModule>)? onSubmit;
  const ExtraExpendView({super.key, this.onSubmit});

  @override
  State<ExtraExpendView> createState() => _ExtraExpendViewState();
}

class _ExtraExpendViewState extends State<ExtraExpendView> {
  List<ExtraExpendModule> inputList = [];
  List<ActiveUser> convertedList = [];

  @override
  void initState() {
    super.initState();
    inputList.add(ExtraExpendModule()); // Add one by default
  }

  void addRow() {
    setState(() {
      inputList.add(ExtraExpendModule());
    });
  }
